...
instances (N=2): what stayed the same is the invariant, what changed is the variable.
...
## The core model (the invariant spine — identical across instances)

A **Knowledge Base (KB)** whose *structure* makes it executable, compiled into frozen
skill artifacts:
...
- **Pipeline** — an ordered sequence of Molds composing an end-to-end task.
...
## The gate (the headline *variable* — what differs across instances)

**The gate** is the external, non-self-certifying check that stands between authored
knowledge and a *trusted* artifact or result. Every Foundry has a gate; what the gate
*is* differs per instance. This is the axis on which the pattern's abstraction is earned.

- **Galaxy Workflow Foundry — a deterministic CLI gate ("the rails").** `gxwf` parses and
  validates gxformat2 and tool steps. Failure modes (hallucinated tool IDs, dropped
  `+galaxyN` revisions, bad `tool_state`) are caught *mechanically*, not by prose caveats.
- **Statistical Genomics Foundry — an empirical referee gate ("doing never
  self-certifies").** No CLI can decide "is this statistical method valid," so the gate is
  ...
  the null, simulation-under-truth, calibration).

The umbrella term on this site is **the gate**; "rails" and "referee" are the two
instance realizations. The generalization is: *a deterministic CLI is one kind of gate;
an empirical referee Mold is another.*
...
- **Provenance** — which specific claim is real and where it came from.
- **The gate / referee** — the *enforced* empirical check, not an optional caveat.
  Knowing ≠ gating: a model may *mention* a caveat, but only a gate makes doing *not
  terminate* until a referee clears it.

Restating canonical knowledge is a commodity. *Traceable, refereed* knowledge is not.
...
**enforcement** (a gate) and **traceability** (provenance). "Knowing ≠ gating." This is the
