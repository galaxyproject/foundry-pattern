...
So this page is the evidence. Instance #1 is the [[galaxy-workflow-foundry]] (convert workflows from papers, Nextflow, and CWL into validated `gxformat2`). Instance #2 is the [[statistical-genomics-foundry]] (keep an LLM doing statistical genomics both productive *and* honest). Different domains, different authors' intent, different failure modes. One caveat up front: instance #1 has a working source-to-cast path; instance #2 has a typed corpus, validation, and authored Molds, but no caster or cast artifacts. So this is a diff between one fully exercised substrate and one whose source side is implemented while its delivery boundary remains designed. Lay their architectures side by side and ask the only question that matters: **what had to stay, and what was free to change?**
...
- **Mold → Cast → provenance.** Both keep the architecture: a Mold is a typed reference manifest, a Cast is deterministic compilation into a frozen artifact, and provenance records its inputs. Instance #1 runs that path and emits `_provenance.json`; instance #2 has implemented the typed Mold side and documented the cast contract, but has not built the caster or emitted provenance yet.
...
- **The external-check loop shape.** Instance #1 runs `author → validate → fix`; instance #2's authored referee Molds specify `analyze → referee → revise`. The intended topology is the same—something *external to the work* can force a fix—even though the second runtime path is not packaged yet and the external instrument differs completely.
...
These are honest *lightenings*, not oversights — instance #2 says so explicitly. They remain judgment calls: the authored Molds now provide evidence that prose deliverables dominate, but the decision should be retested as the corpus and artifact handoffs grow.

## Added — net-new in instance #2

- **An empirical referee as the external check.** Instance #1's check is pure infrastructure: a deterministic CLI parses and either accepts or rejects. Instance #2 has no CLI that can decide "is this statistical method valid," so its check becomes an authored referee Mold that both *critiques* (reasons about known invalidity patterns) and *calibrates* (constructs and runs an empirical check: permutation under the null, simulation under known truth). Once casting exists, that check moves from infrastructure to deliverable. This is instance #2's domain extension, not a new universal part.
