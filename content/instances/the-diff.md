...
- **Mold → Cast → provenance.** Both keep the Mold (a typed reference manifest), the Cast (deterministic compilation into a frozen, self-contained artifact with no link back), and the `_provenance.json` emitted beside every artifact (source hash, target, resolved-reference hashes, timestamp). Instance #2 calls provenance "the single cleanest distinction from prior art."
